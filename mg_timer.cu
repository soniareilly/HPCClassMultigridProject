...
for (int N = 32; N <= 4096; N<<=1){
    ...
    Check_CUDA_Error("Failed timer start setup\n");
    Check_CUDA_Error("Failed timer stop setup\n");
    ...
    Check_CUDA_Error("Failed to start timer\n");
    ...
    Check_CUDA_Error("Failed to stop timer\n");
    cudaEventSynchronize(stopEvent);
    Check_CUDA_Error("Failed to synchronize timer\n");
    cudaEventElapsedTime(&ms, startEvent, stopEvent);
    printf("Time elapsed for grid size %d: %g ms\n", N, ms);

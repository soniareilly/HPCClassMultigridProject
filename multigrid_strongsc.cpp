...
int main(){
    ...
    int N = 1024;                    // Finest grid size. MUST BE A POWER OF 2
    ...
    FILE *f = fopen("strong_scale.txt","w");

    double error;
    // tt = omp_get_wtime();
    int ntr; // number of threads
    for (ntr=1; ntr<=16; ntr++) {
        ...
        // if (ntr==1) memcpy(uTomp, uTref, (N+1)*(N+1)*sizeof(double));
        // else {
